...
ms:contentKeyID: 18126658
ms:mtpsurl: 'https://technet.microsoft.com/cs-cz/library/Cc708558(v=WS.10)'
---

Migrating from Windows Internal Database to SQL Server 2005
===========================================================

This topic explains how to migrate the WSUS database (SUSDB) from a Interní databáze systému Windows instance (installed by default during WSUS setup) to a full version of Microsoft SQL Server 2005.

Reasons to migrate the WSUS database to SQL Server 2005
-------------------------------------------------------

-   If you chose to use Interní databáze systému Windows as the WSUS database when you set up your WSUS server, you may want to upgrade the database engine to a full installation of SQL Server 2005. SQL Server 2005 allows you to administer the WSUS database through the SQL Server Management Studio.
...
-   Migrating the Interní databáze systému Windows database to a SQL Server 2005 instance running on the WSUS server
-   Migrating the Interní databáze systému Windows database to a SQL Server 2005 instance running on another server (remote SQL)
...
Use the following steps to migrate the WSUS database from a Interní databáze systému Windows instance to a SQL Server 2005 instance.
...
1.  Attach **SUSDB** to the destination SQL instance.
    ...
    -   In the **Attach Databases** box, under **Databases to attach**, browse to the location of the susdb.mdf file (by default this is **C:\\WSUS\\UpdateServicesDbFiles** if you installed Interní databáze systému Windows), and then click **OK**.
...
The goal of this scenario is to take the WSUS database (SUSDB) running in a Interní databáze systému Windows instance on the WSUS server and move and upgrade it to a SQL Server 2005 instance running on a remote server. Only a full SQL Server 2005 database may be used in a remote SQL installation. Note that in each step, where appropriate, it is noted on which server you must perform the procedures.
