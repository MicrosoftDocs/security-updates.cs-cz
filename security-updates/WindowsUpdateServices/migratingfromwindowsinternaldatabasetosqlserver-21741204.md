...
ms:contentKeyID: 21741204
ms:mtpsurl: 'https://technet.microsoft.com/cs-cz/library/Dd939918(v=WS.10)'
---

Migrating from Windows Internal Database to SQL Server
======================================================

This topic explains how to migrate the Windows Server Update Services (WSUS) database from a Interní databáze systému Windows instance (installed by default during WSUS setup) to a full version of Microsoft SQL Server 2008 or SQL Server 2005 SP2.

Why Migrate the WSUS Database to SQL Server
-------------------------------------------

If you chose to use Interní databáze systému Windows as the WSUS database when you set up your WSUS server, you may want to upgrade the database engine to a full installation of SQL Server 2008 or SQL Server 2005 SP2. SQL Server lets you administer the WSUS database through the SQL Server Management Studio.
...
-   Migrating the Interní databáze systému Windows database to an instance of SQL Server 2008 or SQL Server 2005 SP2 that is running on the WSUS server
...
<thead>
<tr class="header">
<th><img src="images/Dd939918.Warning(WS.10).gif" />Upozornění</th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
Use the following steps to migrate the WSUS database from a Interní databáze systému Windows instance to an instance of SQL Server 2008 or SQL Server 2005 SP2.
...
5.  Run the following SQL command to detach the WSUS database (SUSDB) from the Interní databáze systému Windows instance, by using the **sqlcmd** utility. For more information about the **sqlcmd** utility, see [sqlcmd Utility](http://go.microsoft.com/fwlink/?linkid=81183) (http://go.microsoft.com/fwlink/?LinkId=81183).
...
8.  In the **Attach Databases** box, under **Databases to attach**, locate the susdb.mdf file (by default, this is **C:\\WSUS\\UpdateServicesDbFiles** if you installed Interní databáze systému Windows), and then click **OK**.
...
13. Verify that the database migration was successful by opening the WSUS administrative console. (Click **Start**, click **Administrative Tools**, and then click **Microsoft Windows Server Update Services 3.0**.)
    ...
    <thead>
    <tr class="header">
    <th><img src="images/Dd939918.note(WS.10).gif" />Poznámka</th>
    </tr>
    </thead>
    <tbody>
    <tr class="odd">
...
The goal of this scenario is to take the WSUS database running in a Interní databáze systému Windows instance on the WSUS server and move and upgrade it to an instance of SQL Server 2008 or SQL Server 2005 SP2 that is running on a remote server. Only a full SQL Server 2008 or SQL Server 2005 SP2 database may be used in a remote SQL installation. Each step, where appropriate, indicates the server on which you perform the procedure.
...
-   You cannot use Interní databáze systému Windows for database software on the back-end server.
...
-   WSUS with Interní databáze systému Windows
...
5.  On the back end server:
    ...
    2.  In the **Attach Databases** box, under **Databases to attach**, locate the susdb.mdf file (by default this is **C:\\WSUS\\UpdateServicesDbFiles** if you installed Interní databáze systému Windows), and then click **OK**.
